...
ALLOWED_METRICS = {"duration", "calories", "sessions"}
...
update_parser.add_argument("description", type=str, required=False, location=("json", "form"))
update_parser.add_argument("target_value", type=_positive_int, required=False, location=("json", "form"))
update_parser.add_argument("period", type=str, required=False, location=("json", "form"))
update_parser.add_argument("metric", type=str, required=False, location=("json", "form"))
update_parser.add_argument("exercise_type_id", type=int, required=False, location=("json", "form"))
update_parser.add_argument("start_date", type=str, required=False, location=("json", "form"))
update_parser.add_argument("end_date", type=str, required=False, location=("json", "form"))
...
class GoalList(Resource):
    ...
    def get(self):
        ...
        if args.get("exercise_type_id") is not None:
            q = q.filter(Goal.exercise_type_id == args["exercise_type_id"])
        ...
        if start_f or end_t:
            if start_f and end_t:
                q = q.filter(or_(Goal.start_date == None, Goal.start_date <= end_t))  # noqa: E711
                q = q.filter(or_(Goal.end_date == None, Goal.end_date >= start_f))    # noqa: E711
            elif start_f:
                q = q.filter(or_(Goal.end_date == None, Goal.end_date >= start_f))   # noqa: E711
            elif end_t:
                q = q.filter(or_(Goal.start_date == None, Goal.start_date <= end_t)) # noqa: E711
...
class GoalDetail(Resource):
    ...
    def put(self, goal_id: int):
        ...
        if g.exercise_type_id:
            et = ExerciseType.query.filter_by(id=g.exercise_type_id, user_id=user_id).first()
